...
One possible layout for your widgets;
   ...
   lib/MyApp/Form
   lib/MyApp/Form/Widget   (contains MyWidget.pm)
   lib/MyApp/Form/Widget/Form
   lib/MyApp/Form/Widget/Field
...
you should use C<< $field->result >> to access the current value and pass a
result object to 'fif' (fill-in-form) in field widgets, so that they will work 
for both form and result rendering.

The basic template for a field widget:
