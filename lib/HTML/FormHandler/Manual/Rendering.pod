...
supplied form rendering roles are in HTML::FormHandler::Widget::Form (Simple
and Table). By default the 'HTML::FormHandler::Widget::Form::Simple' role is used.
Specify a different form rendering widget (or other rendering class
containing a 'render' sub) by including the role in your form class:

   package MyApp::Form;
   with 'HTML::FormHandler::Widget::Form::Table';
   -- or --
   with 'MyApp::Form::Render::Special';

The 'wrapper' for field rendering can be specified with 'widget_wrapper'.
...
'label_attr' and attributes for the wrapper with 'wrapper_attr'. See the
...
The 'tags' hashref attribute has been provided to allow customizing
...
that it uses; you can add your own for your custom rendering code.
    ...
    form_wrapper_attr -- attributes for the wrapping tag (fieldset, div, table...)

    compound_wrapper -- put a wrapper around compound fields
    compound_wrapper_tag -- the tag for the compound wrapper (default 'fieldset')
...
other element, you can set the wrapper_start and wrapper_end to ''.
   ...
   has_field 'baz' => ( tags => { wrapper_start => '',
        wrapper_end => '' } );
...
widget 'tags' in a form to apply them to all the fields:

   package MyApp::Form;
   ....
   has '+tags' => ( default => sub { {wrapper_start => '<p>',
      wrapper_end => '</p>' } } );
