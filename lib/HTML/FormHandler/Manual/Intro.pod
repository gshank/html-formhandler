...
not actually create the HTML code themselves, although there is an example of a
rendering role.
...
L<HTML::FormHandler::Model::DBIC> and L<HTML::FormHandler>.
...
There is often no need to check the 'validated' flag, since that is the 
return value from the 'process' method, although it can be useful if you are 
doing anything between the process call and displaying the form, such as setting
...
'fif'. The field's 'html_name' convenience attribute will return this
...
When data is passed in to validate the form, the input in copied into the
...
the field 'validate' method is called.

A transformation action with the 'to_fif' flag set will cause the value
produced by this transformation to be copied to the fill-in-form slot, so
that a 'canonical' form of the input can be presented to the user.
...
as a DateTime, then you will also need a transformation method to reformat
...
the entire form is validated.  This method is executed whether or not the form 
...
has lots of examples. Here is an example of a test script for a DBIC form:
   ...
   ok( $form->process, 'now form validates' );
