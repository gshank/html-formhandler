...
For many forms, if L<HTML::FormHandler::Render:Simple> is included as a form role,
...
A DBIC form generator is installed with FormHandler. See 
L<HTML::FormHandler::Generator::DBIC>.
...
These is the same 'value' structure that FormHandler uses to update
the database.

=head1 Form Models 

For a database form, use a model base class that interfaces with the 
database, such as L<HTML::FormHandler::Model::DBIC>.
...
as a DateTime, then you will also need a deflation to reformat
