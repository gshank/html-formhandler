...
performed. The form's 'validate' method, however, will always be called.
...
Provide a validation function to check. Note that the field
is passed in as the second argument, to allow simple functions
to work properly.
...
The message for the above checks can also be a an arrayref or coderef.
