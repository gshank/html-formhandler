...
=head1 Dynamically change the active fields 
...
'setup_form'.
   ...
   after 'setup_form' => sub {
      ...
      if( <some condition> ) 
      {
         ...
         $self->field('bar')->clear_inactive;
      }
   }
   
=head1 Add custom attributes to FormHandler fields
...
attributes to L<HTML::FormHandler::Field> in your Form's BUILD subroutine.
...
can create a base form object with a BUILD that applies the role, or turn the
code into a role with a method modifier on BUILD.
