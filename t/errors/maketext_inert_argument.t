...
# It contains no bracket metacharacters, so it must still be looked up and
# translated -- this is what distinguishes diverting only parseable text from
...
# A payload carried inside an exception object must be shown, not dispatched.
{
    ...
    lives_ok { $form->process( params => { f => 'x' } ) }
        'an exception object stringifying to a bracket group does not kill form processing';
    my $errors = errors_of($form);
    like $errors, qr/\Q[sprintf,%20d,7]\E/,
...

{
    ...
    is errors_of($form), 'TRANSLATED OK',
        'a type message with no bracket metacharacters is still looked up in the lexicon and translated';
}
