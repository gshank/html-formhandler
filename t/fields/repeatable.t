...
$field->init_state;
is_deeply( $field->contains->get_wrapper_attr('class'), ['hfh', 'repinst'], 'attribute set' );

{
    ...
    has_field 'my_records' => ( type => 'Repeatable', num_when_empty => 2,
        init_contains => { wrapper_attr => { class => ['hfh'] } },
    );
...
ok( $form, 'form built' );
is_deeply( $form->field('my_records')->contains->get_wrapper_attr('class'), ['hfh'], 'worked in form');
